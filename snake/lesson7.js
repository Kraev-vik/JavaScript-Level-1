...
var BOMB_SPEED = 3500; //Интервал появления бомб
...
function move() {
    ...
    if (direction == 'x-') {
        coord_x == 0 ? coord_x = FIELD_SIZE_X : coord_x;
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x - 1))[0];
    }
    else if (direction == 'x+') {
        coord_x == FIELD_SIZE_X - 1 ? coord_x = -1 : coord_x;
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x + 1))[0];
    }
    else if (direction == 'y+') {
        coord_y == 0 ? coord_y = FIELD_SIZE_Y : coord_y;
        new_unit = document.getElementsByClassName('cell-' + (coord_y - 1) + '-' + (coord_x))[0];
    }
    else if (direction == 'y-') {
        coord_y == FIELD_SIZE_Y - 1 ? coord_y = -1 : coord_y;
        new_unit = document.getElementsByClassName('cell-' + (coord_y + 1) + '-' + (coord_x))[0];
    }
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood(0);
        score++;
        document.querySelector(".curent_result").innerHTML = "<h4> Текущий результат: " + score + "</h4>";
    }
    return check;
}
 ...
 */
function createFood(numClass = 0) {
    var foodCreated = false;
    var arrClass = ["food-unit", "bomb-unit"];
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        if (!food_cell_classes.includes('snake-unit')) {
            ...
            food_cell.setAttribute('class', classes + arrClass[numClass]);
            foodCreated = true;
        }
    }
    if (numClass == 1) {
        return food_cell_classes[1];
    }

}

function createBomb() {
    bomb.push(createFood(1));
    if (bomb.length > 2) {
        document.querySelector(".game-table-cell." + bomb[0] + ".bomb-unit").classList.remove("bomb-unit");
        bomb.splice(0, 1);
    }
}
